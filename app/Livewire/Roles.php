...
class Roles extends Component
{
    ...
    public function save()
    {
        $rules = $this->rules;
        if ($this->editingId) {
            // editing: name must be unique except for the current record
            $rules['name'] = ['required','string','max:255', Rule::unique('roles','name')->ignore($this->editingId)];
        } else {
            // creating: enforce uniqueness on name to match DB unique constraint
            $rules['name'] = ['required','string','max:255', Rule::unique('roles','name')];
        }
