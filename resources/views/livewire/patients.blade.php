<div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 items-center">
            ...
            <div class="col-span-1 flex justify-center sm:justify-end">
                <div class="flex w-full gap-2">
                    ...
                    <a href="{{ route('patients.trashed') }}" class="flex-1 inline-flex items-center justify-center bg-gray-700 text-white px-4 py-2 rounded h-10">Eliminados</a>
                </div>
            </div>
        </div>
    </div>
    ...
    @if($showForm)
        <div class="p-4 border rounded bg-white dark:bg-gray-800 mb-4">
            <div x-data="{ create: @entangle('createUser') }" class="grid grid-cols-1 md:grid-cols-2 gap-3">
                @unless($patientId)
                            ...
                            @unless($patientId)
                                <div class="flex-1" x-show="create" x-cloak style="display:none;">
                                    <select wire:model.defer="user_id" class="mt-1 block w-full border rounded px-2 py-1 bg-white dark:bg-gray-900 dark:text-gray-200">
                                        ...
                                        @foreach(($availableUsers ?? []) as $id => $label)
                ...
                <tbody class="md:table-row-group">
                    @foreach($patients as $patient)
                        <tr class="block md:table-row mb-3 md:mb-0 bg-white dark:bg-gray-800 rounded-lg md:rounded-none shadow-sm md:shadow-none overflow-hidden">
                            <td class="px-3 py-1">{{ $patient->id }}</td>
                            <td class="px-3 py-1">{{ optional($patient->user)->name ?? '—' }}</td>
                            <td class="px-3 py-1">{{ $patient->birthdate?->toDateString() ?? '—' }}</td>
                            <td class="px-3 py-1">{{ $patient->phone ?? '—' }}</td>
                            <td class="px-3 py-1">{{ Str::limit($patient->notes, 60) }}</td>
                            <td class="px-3 py-1">{{ $patient->gender ? ucfirst($patient->gender) : '—' }}</td>
                            <td class="px-3 py-1">
                                <div class="flex flex-col md:flex-row gap-2">
                                    <button wire:click="edit({{ $patient->id }})" class="px-3 py-1 rounded text-white bg-yellow-400 w-full md:w-auto">Editar</button>
                                    @if($patient->trashed())
                                        <button onclick="confirmAction('restore', {{ $patient->id }})" class="px-3 py-1 rounded text-white bg-green-600 w-full md:w-auto">Restaurar</button>
                                    @else
                                        <button onclick="confirmAction('delete', {{ $patient->id }})" class="px-3 py-1 rounded text-white bg-red-600 w-full md:w-auto">Eliminar</button>
                                    @endif
                                </div>
                            </td>
                        </tr>
