<div>
    <!-- Responsive trashed roles list: cards on mobile, table on desktop -->
    <div class="mb-4">
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 items-center">
            <div class="col-span-1 flex justify-center sm:justify-start">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100 text-center sm:text-left">ROLES ELIMINADOS</h2>
            </div>
            <div class="col-span-1 hidden sm:block"></div>
            <div class="col-span-1 flex justify-center sm:justify-end">
                <a href="<?php echo e(route('roles.index')); ?>" class="inline-flex items-center justify-center px-4 py-2 rounded h-10 bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-gray-100">Volver a roles</a>
            </div>
        </div>
    </div>
            ...
            <table class="min-w-full w-full table-auto text-gray-900 dark:text-gray-100">
                ...
                <tbody class="md:table-row-group">
                        ...
                        <tr class="block md:table-row mb-3 md:mb-0 bg-white dark:bg-gray-800 rounded-lg md:rounded-none shadow-sm md:shadow-none overflow-hidden">
                            ...
                            <td class="px-3 py-1 block md:table-cell align-top">
                                <div class="md:hidden flex justify-between items-start">
                                    ...
                                    <span class="text-sm"><?php echo e($role->deleted_at?->diffForHumans()); ?></span>
                                </div>
                                <div class="hidden md:block">
                                    <span class="block text-sm"><?php echo e($role->deleted_at?->diffForHumans()); ?></span>
                                </div>
                            </td>
                            <td class="px-3 py-1 block md:table-cell align-top">
                                <div class="mt-2 md:mt-0">
                                    <div class="md:hidden flex justify-between items-center">
                                        ...
                                        <div class="flex items-center space-x-2">
                                            <button onclick="confirmActionRoles('restore', <?php echo e($role->id); ?>)" class="inline-flex items-center justify-center w-9 h-9 rounded-full text-white bg-gradient-to-r from-green-500 to-green-600">R</button>
                                        </div>
                                    </div>
                                    <div class="hidden md:flex md:flex-row md:items-center md:gap-2">
                                        <button onclick="confirmActionRoles('restore', <?php echo e($role->id); ?>)" class="w-full md:w-auto text-sm px-3 py-1 rounded text-white bg-gradient-to-r from-green-500 to-green-600">Restaurar</button>
                                    </div>
                                </div>
                            </td>
                        </tr>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                        <tr>
                            <td colspan="5" class="px-4 py-6 text-center text-sm text-gray-500">No hay roles eliminados</td>
                        </tr>
    ...
    <!--[if BLOCK]><![endif]--><?php if(session()->has('toast')): ?>
        <script>window.dispatchEvent(new CustomEvent('showToast',{detail:<?php echo json_encode(session('toast'), 15, 512) ?>}));</script>
    <?php endif; ?><!--[if ENDBLOCK]><![endif]-->
    <!-- Confirmation modal for trashed roles -->
<div id="confirm-modal-trashed-roles" class="fixed inset-0 z-50 hidden flex items-center justify-center bg-black bg-opacity-40 backdrop-blur-sm backdrop-filter p-4">
    <div id="confirm-panel-trashed-roles" class="transform transition-opacity transition-transform duration-200 ease-out opacity-0 -translate-y-2 scale-95 bg-white dark:bg-gray-800 rounded-lg shadow-2xl max-w-lg w-full overflow-hidden">
        <div class="p-6 text-center">
            <div id="confirm-icon-trashed-roles" class="mx-auto flex items-center justify-center h-20 w-20 rounded-full bg-green-100 dark:bg-green-900 mb-4">
                <svg id="confirm-icon-svg-trashed-roles" xmlns="http://www.w3.org/2000/svg" class="h-10 w-10 text-green-600 dark:text-green-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
            </div>
            <h3 id="confirm-title-trashed-roles" class="text-xl font-semibold text-gray-900 dark:text-gray-100">¿Deseas restaurar este rol?</h3>
            <div id="confirm-text-trashed-roles" class="mt-2 text-sm text-gray-600 dark:text-gray-300">El rol volverá a estar activo.</div>
        </div>
        <div class="px-6 pb-6 pt-0 bg-gray-50 dark:bg-gray-900 flex justify-center gap-3">
            <button id="confirm-cancel-trashed-roles" class="px-4 py-2 rounded-md bg-white border border-gray-200 text-gray-700 hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-200">Cancelar</button>
            <button id="confirm-ok-trashed-roles" class="px-4 py-2 rounded-md bg-green-600 text-white hover:bg-green-700 focus:outline-none">Confirmar</button>
        </div>
    </div>
</div>
...
<script>
    (function(){
        const modal = document.getElementById('confirm-modal-trashed-roles');
        const panel = document.getElementById('confirm-panel-trashed-roles');
        const btnOk = document.getElementById('confirm-ok-trashed-roles');
        const btnCancel = document.getElementById('confirm-cancel-trashed-roles');
...
<?php /**PATH C:\laragon\www\medicall\resources\views/livewire/trashed-roles.blade.php ENDPATH**/ ?>